...
static void remove_shm(void) {
  ...
  shmctl(shm_id_testprog, IPC_RMID, NULL);
}
...
EXP_ST void setup_shm(void) {
  ...
  shm_id_testprog = shmget(SHM_AFL_N_TEST_PROG, 256, IPC_CREAT | 0666);
  ...
  // shm_testprog_str = alloc_printf("%d", shm_id_testprog);
  ...
  if (!dumb_mode) {
      ...
      // setenv(SHM_AFL_N_TEST_PROG, shm_testprog_str, 1);
  }

  ck_free(shm_str);
  // ck_free(shm_testprog_str);

  trace_bits = shmat(shm_id, NULL, 0);
  test_input_file = shmat(shm_id_testprog, NULL, 0);

  if (!trace_bits) PFATAL("shmat() failed");
  if(!test_input_file) PFATAL("shmat() failed for test_input_file");
...
static u8 save_if_interesting(char** argv, void* mem, u32 len, u8 fault) {
  ...
  if (fault == crash_mode) {
    ...
    memcpy(test_input_file, fn, strlen(fn) + 1);
